...
import org.xtext.adaptui.validation.AbstractAdaptUIValidator;
...
public class AdaptUIValidator extends AbstractAdaptUIValidator {
  @Check
  public void isProviderNotEmpty(final Provider provider) {
    String _name = provider.getName();
    boolean _equals = Objects.equal(_name, "");
    if (_equals) {
      this.error("Provider Name shall not be empty", AdaptUIPackage.Literals.PROVIDER__NAME);
    }
  }
        ...
        for (final Entity secEnt : _entity_1) {
          {
            ...
            if ((Objects.equal(firstEnt.getName(), secEnt.getName()) && (i != j))) {
              this.error("Entity names must be unique in a Context Model.", AdaptUIPackage.Literals.CONTEXT_MODEL__ENTITY);
            }
          }
        }
      }
        ...
        for (final Property secProp : _property_1) {
          {
            ...
            if ((Objects.equal(firstProp.getName(), secProp.getName()) && (i != j))) {
              this.error("Property names must be unique in an Entity.", AdaptUIPackage.Literals.ENTITY__PROPERTY);
            }
          }
        }
      }
        ...
        for (final Provider secProv : _provider_1) {
          {
            ...
            if ((Objects.equal(firstProv.getName(), secProv.getName()) && (i != j))) {
              this.error("Provider names must be unique in a Context Model.", AdaptUIPackage.Literals.CONTEXT_MODEL__PROVIDER);
            }
          }
        }
      }
